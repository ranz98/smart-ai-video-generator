document.addEventListener('DOMContentLoaded', function() {
    ...
    generatePromptsBtn.addEventListener('click', async function() {
        ...
        saveScriptBtn.style.display = 'none'; // Ensure save button is hidden
    ...
    generateImagesBtn.addEventListener('click', async function() {
        ...
        // Clear previous images and hide create video button
        imageGrid.innerHTML = '';
        createVideoBtn.style.display = 'none';
        ...
        for (let i = 0; i < prompts.length; i++) {
            ...
            const filename = `${uniqueID}_${randomString}_${imageNumber}`; // Initial filename for this slot
            ...
            container.innerHTML = `
                ...
                <img src="" alt="${prompt}" class="generated-image" data-prompt="${prompt}" data-filename="${filename}"> <div class="prompt-display">${prompt}</div>
            ...
            // Pass the generated filename to the generateImage function
            generationPromises.push(generateImage(prompt, i, isDemoMode, filename)); // <<< Pass the generated filename
        }
    ...
    function setupImageGridListeners() {
         ...
         // (though clearing imageGrid.innerHTML should handle this, it's safer)
    ...
    function handleImageGridClick(event) {
         ...
         if (regenerateBtn && !regenerateBtn.disabled) { // Check if it's the regenerate button and not disabled
             ...
             const filename = imgElement.dataset.filename; // <<< Get the stored filename
             ...
             if (prompt && index >= 0 && container && filename) { // Ensure prompt, index, container, AND filename exist
                  console.log(`User clicked regenerate for image ${index + 1} (Filename: ${filename}) with prompt: "${prompt}"`);
                  ...
                  // Call generateImage for this specific image, passing the STORED filename
                  generateImage(prompt, index, isDemoMode, filename) // <<< Pass the stored filename
                       .then(() => {
                  ...
                  console.warn("Cannot regenerate image: prompt, index, container, or filename missing, or button disabled.");
             }
             return; // Stop processing event for this click
         }
     }
  ...
  createVideoBtn.addEventListener('click', async function() {
    ...
    // Get all successfully loaded image filenames in their current order
    // We get filenames from data-filename attribute
    const imageFilenames = Array.from(document.querySelectorAll('.image-container img[style*="display: block"]'))
         .filter(img => img.src && img.dataset.filename) // Ensure image is displayed and has filename data
    ...
    console.log("Using image filenames (in order):", imageFilenames);
    console.log("Using voiceover file:", generatedVoiceoverFilename);


    // --- NEW: Call backend API to create video ---
    try {
         ...
         if (data.success) {
              alert(`Video created successfully! File: ${data.video_path}`); // TODO: Replace with better feedback (maybe a link to download/view)
              console.log("Video creation response:", data);
              // TODO: Provide a link or embed for the generated video using data.video_url or data.video_path
         } else {
    ...
    generateVoiceBtn.addEventListener('click', async function() {
        ...
        try {
            ...
            if (isDemoMode) {
                ...
                const randomString = Math.random().toString(36).substring(2, 8);
                const filename = `demo_${uniqueID}_${randomString}`; // Use uniqueID in demo too, add .mp3 suffix

                // Simulate a successful API call structure if needed,
                // or just use a timeout and alert
                await new Promise(resolve => setTimeout(resolve, 1500)); // Simulate delay

                generatedVoiceoverFilename = filename; // <<< Store the demo filename
                ...
                // Real mode - Call backend API
                 const randomString = Math.random().toString(36).substring(2, 8);
                 const filename = `${uniqueID}_${randomString}`; // Use uniqueID for real filename, add .mp3 suffix
                ...
                const response = await fetch(VOICEOVER_GENERATION_API, { // Use const URL
                    ...
                    body: JSON.stringify({
                        ...
                        save_name: filename // Send generated filename (ensure backend adds .mp3 if needed)
                    })
                });
                ...
                const data = await response.json();
                 // Assuming data.filename returned by backend includes the full filename like 'uniqueID_randomstring.mp3'
                generatedVoiceoverFilename = data.filename; // <<< Store the real filename
        ...
        prompts.forEach((prompt, index) => {
            ...
            // This uses direct event listener as prompt items aren't regenerated as a whole grid
             div.querySelector('.regenerate-prompt').addEventListener('click', async function() {
                ...
                btn.disabled = true; // Disable the specific button
    ...
    async function regeneratePrompt(index) {
        ...
        try {
            ...
            // IMPORTANT: Regenerating a prompt text invalidates previously generated images.
            ...
            imageGrid.innerHTML = ''; // Clear the image grid
             console.log("Image grid cleared and image section hidden after prompt regeneration.");
    ...
    // Now accepts the desired filename as a parameter
    async function generateImage(prompt, index, isDemoMode, filename) { // <<< Added filename parameter
        ...
        // Ensure prompt and filename are set on the image element
        img.dataset.prompt = prompt;
        img.dataset.filename = filename; // Make sure filename is stored
        ...
        try {
            if (isDemoMode) {
                console.log(`Demo Mode: Generating image ${imageNumber} for prompt: "${prompt}" (Filename: ${filename})`);
                // Use demo image URL
                 ...
                 // Real mode - Use the filename provided as a parameter
                 console.log(`Attempting real image generation for prompt ${imageNumber} with save_name: ${filename}`);
                 ...
                 const response = await fetch(IMAGE_GENERATION_API, {
                     ...
                     body: JSON.stringify({
                         ...
                         save_name: filename // <<< USE THE PROVIDED FILENAME
                     })
                 });
                ...
                const dateString = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;
                 ...
                 // Construct the image URL using the PROVIDED filename
                 let imageUrl = `http://localhost//shorts//output//${dateString}//${filename}-0.png`; // Base URL
            ...
            console.error(`Error during generateImage process for image ${imageNumber} (Filename: ${filename}):`, error);
            ...
            // Add retry button to placeholder (if not already there)
             if (!placeholder.querySelector('.btn.btn-sm')) {
                  ...
                  retryBtn.innerHTML = '<i class="fas fa-redo"></i> Retry';
                   // Call generateImage with the same parameters including filename
                   // This call will also apply the cache buster
                  retryBtn.addEventListener('click', () => generateImage(prompt, index, isDemoMode, filename));
